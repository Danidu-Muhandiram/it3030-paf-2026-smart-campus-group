...
export function AdminBookingsPage({
  ...
  const handleApprove = async (bookingId) => {
    setIsLoading(true);
    try {
      await axiosInstance.post(`/bookings/admin/${bookingId}/approve`);
  ...
  const handleReject = async (reason) => {
    ...
    setIsLoading(true);
    try {
      await axiosInstance.post(`/bookings/admin/${rejectingId}/reject`, { reason: reason.trim() });

      // Update local state
      setBookings(prev =>
        prev.map(b =>
          b.id === rejectingId ? { ...b, status: 'REJECTED', rejectionReason: reason } : b
        )
      );
          ...
          <table className="w-full">
            ...
            <tbody>
              {filteredBookings.map((booking) => (
                <tr 
                  key={booking.id} 
                  className={`hover:bg-gray-50 transition-colors border-t ${booking.status === 'PENDING' ? 'cursor-pointer' : ''}`}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            ...
            <p className="text-sm text-gray-600 mb-3">
              Please provide a reason for rejecting this booking (minimum 10 characters).
            </p>
            <textarea
